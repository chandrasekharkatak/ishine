...
public interface FCProjectMilestoneRepository extends JpaRepository<FCProjectMilestone, Long> {
    ...
    @Query("SELECT new com.apmosys.employeeportal.dto.FCProjectMilestoneDTO(pm.id,pm.poId ,pm.projectId , pm.name, pm.description, pm.startDate, pm.endDate, pm.status, pm.remarks,pm.lineItemId,li.name,li.status,pm.poProjectId) \n"
