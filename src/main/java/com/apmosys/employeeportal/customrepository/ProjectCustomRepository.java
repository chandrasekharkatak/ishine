...
public class ProjectCustomRepository {
    ...
    public Slice<ProjectFetchDTO> handleOverboardedAndUnderboardedProjects(
    ...
    private String getAllProjectsQuery(Map<String, String> projectFilter, String sortBy,
		...
		query2.append(" WHERE 1=1 AND NOT EXISTS (SELECT 1 FROM teams t2 INNER JOIN employee_team_mapping etm2 ON t2.team_id = etm2.team_id WHERE t2.project_id = p.project_id AND etm2.active IN (1,2) ) \n")
    ...
    private String getOverboardedAndUnderboardedProjectQuery(Map<String, String> projectFilter, String sortBy,
        ...
        query.append(groupQuery)
                ...
                .append(" SELECT p.project_id, COUNT(DISTINCT etm.emp_id) AS filled_count \n")
                ...
                .append(" select project_id, sum(required_count) as required_count from \n")
                .append(" (SELECT DISTINCT \n")
                ...
                .append(" LEFT JOIN project_po_details ppd ON ppd.project_id = rp.project_id AND DATE(ppd.po_start_date) <= CURRENT_DATE AND (ppd.po_end_date IS NULL OR DATE(ppd.po_end_date) >= CURRENT_DATE) AND ppd.active  = 1 \n")
                .append(" LEFT JOIN po_requirement_mapping prm ON prm.po_id = ppd.po_id AND prm.active = true \n")
                .append(" LEFT JOIN department d on d.name = prm.department) req \n")
                .append(" GROUP BY project_id \n")
                .append(" ) \n")

                .append(" , UnfilledPositions AS ( \n")
                .append(" SELECT rp.project_id, \n")
                .append(" IFNULL(tr.required_count, 0) - IFNULL(fc.filled_count, 0) AS unfilled_count \n")
                .append(" FROM RelevantProjects rp \n")
                .append(" INNER JOIN TotalRequirements tr ON tr.project_id = rp.project_id ")
                .append(" INNER JOIN FilledCounts fc ON fc.project_id = tr.project_id \n")
                .append(" GROUP BY rp.project_id \n")
                .append(" ) \n");

        if (projectStatus.equalsIgnoreCase("OVERBOARDED")) {
            query.append(" SELECT DISTINCT rp.* \n")
                    .append(" FROM RelevantProjects rp \n")
                    .append(" INNER JOIN TotalRequirements tr ON rp.project_id = tr.project_id \n")
                    .append(" INNER JOIN FilledCounts fc ON rp.project_id = fc.project_id \n")
                    .append(" WHERE 1=1 \n")
                    .append(" AND rp.dept_id in :deptIds \n")
                    .append(" AND IFNULL(fc.filled_count, 0) > IFNULL(tr.required_count, 0) \n");
        } else {
            query.append(" SELECT DISTINCT rp.* \n")
                    .append(" FROM RelevantProjects rp \n")
                    .append(" INNER JOIN TotalRequirements tr ON rp.project_id = tr.project_id \n")
    ...
    private String getGeneralProjectFilterQuery(Map<String, String> projectFilter, String sortBy,
		...
		if (projectStatus.equalsIgnoreCase("NOT_STARTED")) {
			query.append(" AND NOT EXISTS (SELECT 1 FROM teams t2 INNER JOIN employee_team_mapping etm2 ON t2.team_id = etm2.team_id WHERE t2.project_id = p.project_id AND etm2.active IN (1,2) ) \n")
					.append(" AND p.active = 'true' AND p.is_draft_project IS NULL \n")
					.append(" AND (p.status != 'Completed' or p.status IS NULL) \n")
					.append(" AND (DATE(ppd.po_end_date) > CURDATE() OR ppd.po_end_date IS NULL ) \n");
