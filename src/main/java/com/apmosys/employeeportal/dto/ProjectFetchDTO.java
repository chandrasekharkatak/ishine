...
public class ProjectFetchDTO {
	...
	public ProjectFetchDTO(Integer projectId, Timestamp createdOn, String projectName, String state, Integer clientId,
			...
			String projectStartDate, String apmosysRM, String clientRM, String deptId, Boolean isRenewable,
			String status,
	...
	public static ProjectFetchDTO projectDetailsBaseColumn(Object[] row) {
		...
		projectFetchDTO.projectStartDate = TypeConversionUtil.getSafeString(row[6]);
		projectFetchDTO.projectEndDate = TypeConversionUtil.getSafeString(row[7]);
		...
		projectFetchDTO.projectManager = TypeConversionUtil.getSafeString(row[13]);
		projectFetchDTO.active = TypeConversionUtil.getSafeString(row[14]);
		projectFetchDTO.status = TypeConversionUtil.getSafeString(row[15]);
		projectFetchDTO.projectStatus = TypeConversionUtil.getSafeString(row[16]);
		projectFetchDTO.syncProject = TypeConversionUtil.getSafeString(row[17]);
		projectFetchDTO.createdBy = TypeConversionUtil.safeParseLong(row[18]);
		projectFetchDTO.updatedBy = TypeConversionUtil.safeParseLong(row[19]);
		projectFetchDTO.draftStatus = TypeConversionUtil.getSafeString(row[20]);
		projectFetchDTO.projectViewId = TypeConversionUtil.getSafeString(row[21]);
		projectFetchDTO.createdOn = row[22] != null ? (row[22] instanceof Timestamp ? ((Timestamp) row[22]) : null)
				: null;
		projectFetchDTO.updatedOn = row[23] != null
	...
	// projectFetchDTO.projectId = TypeConversionUtil.safeParseInt(row[0]);
	// projectFetchDTO.name = TypeConversionUtil.getSafeString(row[1]);
	// projectFetchDTO.internalProjectType =
	// TypeConversionUtil.getSafeString(row[2]);
	// projectFetchDTO.poProjectType = TypeConversionUtil.getSafeString(row[3]);
	// projectFetchDTO.poNo = TypeConversionUtil.getSafeString(row[4]);
	// projectFetchDTO.poProjectId = TypeConversionUtil.safeParseLong(row[5]);
	// projectFetchDTO.projectStartDate = TypeConversionUtil.getSafeString(row[6]);
	...
	// projectFetchDTO.createdOn = row[23] != null ? (row[23] instanceof Timestamp ?
	// ((Timestamp) row[23]) : null)
	// : null;
	// projectFetchDTO.updatedOn = row[24] != null
	// ? (row[24] instanceof Timestamp ? ((Timestamp) row[24]).toLocalDateTime() :
	// null)
	// : null;
	// projectFetchDTO.projectCompletionDate =
	// TypeConversionUtil.getSafeString(row[25]);
	// return projectFetchDTO;
	// }
