...
//	TimesheetDocumentDetailsNew findByDocIdAndActive_old(Long docId, Boolean active);
//	TimesheetDocumentDetailsNew findByDocIdAndFinalFlag_old(Long docId, Boolean finalFlag);
//	TimesheetDocumentDetailsNew findTopByTimesheetIdAndActive_old(Long timesheetId,Boolean active);
	...
	// ========== UPDATED: New query using _new entities ==========
	@Query("SELECT new com.apmosys.employeeportal.dto.TimesheetDocumentDetailsDTO(" +
            "tdd.docId, tdd.docName, tdd.timesheetId, et.empId, tdd.active, " +
            "COALESCE(csm.status, ''), '', '', tdd.finalFlag, tdd.bulkApprovedDocId) " +
