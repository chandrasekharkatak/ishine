...
public interface EmployeePerformanceRepository extends JpaRepository<EmployeePerformance, Long> {
	...
	@Query(nativeQuery = true, value = "SELECT ep.manager_id, ep.manager_remarks, ep.final_rating, ep.manager_rating, ep.hod_rating, ep.hr_rating, "
			...
			+ "ep.manager_review_date, ep.hod_approval_date, ep.hod_rejected_date, ep.hr_review_date, "
			+ "e_emp.name AS employee_name, e_emp.employeement_id AS employee_employment_id "
			+ "FROM employee_performance ep "
			+ "LEFT JOIN employee e_emp ON ep.emp_id = e_emp.emp_id "
	...
	@Query(nativeQuery = true,value="SELECT DISTINCT\n"
			...
			+ "    ep.hod_rating,\n"
			+ "    ep.hr_rating,\n"
			+ "    e_m_perf.name AS performance_manager_name,\n"
			+ "    e_hr_perf.name AS hr_rater_name\n"
			+ "FROM \n"
			+ "    employee e\n"
			+ "INNER JOIN \n"
			...
			+ "    AND ep.quarter_id = (SELECT MAX(qc2.quarter_id) FROM quater_cycle qc2 WHERE qc2.is_enable = 1 AND qc2.is_active = 1)\n"
			+ "LEFT JOIN employee e_m_perf ON ep.manager_id = e_m_perf.emp_id\n"
			+ "LEFT JOIN employee e_hr_perf ON ep.hr_id = e_hr_perf.emp_id\n"
			+ "LEFT JOIN \n"
	...
	/** Latest performance row per employee for active quarter: role ratings, remarks, rater names (export / table merge). */
	@Query(nativeQuery = true, value = "SELECT ep.emp_id, ep.manager_rating, ep.hod_rating, ep.hr_rating, "
			+ "ep.manager_remarks, ep.hod_remarks, ep.hr_remarks, "
			+ "e_m.name AS manager_rater_name, e_hr.name AS hr_rater_name "
			+ "FROM employee_performance ep "
			+ "LEFT JOIN employee e_m ON ep.manager_id = e_m.emp_id "
			+ "LEFT JOIN employee e_hr ON ep.hr_id = e_hr.emp_id "
			+ "INNER JOIN ( "
			+ "  SELECT emp_id, MAX(employee_performance_id) AS max_ep_id "
			+ "  FROM employee_performance "
			+ "  WHERE quarter_id = (SELECT MAX(qc.quarter_id) FROM quater_cycle qc WHERE qc.is_enable = 1 AND qc.is_active = 1) "
			+ "  GROUP BY emp_id "
			+ ") latest ON ep.employee_performance_id = latest.max_ep_id")
	List<Object[]> findRoleRatingsAndRemarksByEmpIdForActiveQuarter();
