...
//			@Param("date") LocalDate date);
	  ...
	  List<TimesheetDocumentDetails> findAllByTimesheetId(@Param("timesheetId") Long timesheetId);
		...
		@Query(value = "WITH RECURSIVE \n"
				...
				+ "		SELECT DISTINCT et.timesheet_id,et.emp_id,t.project_id,etm.employee_team_map_id,et.date,dm.day_type\n"
				+ "		FROM employee_timesheets_new et\n"
				+ "        JOIN day_type_master_new dm ON et.day_type_id = dm.day_type_id\n"
				+ "		JOIN employee_timesheet_activities_mapping_new etam ON et.timesheet_id = etam.timesheet_id\n"
				...
				+ "                          LEFT JOIN day_type_master_new dm ON et.day_type_id = dm.day_type_id\n"
				+ "                          WHERE et.date = adir.dt\n"
				+ "						  AND UPPER(dm.day_type) IN ('LEAVE','CLIENT HOLIDAY','PUBLIC HOLIDAY','WEEK OFF' , 'APMOSYS HOLIDAY' , 'COMP OFF'))\n"
				+ "	)	,\n"
				...
				+ "'LEAVE','CLIENT HOLIDAY','PUBLIC HOLIDAY','WEEK OFF' , 'APMOSYS HOLIDAY' , 'COMP OFF'\n"
				+ ")\n"
				...
				+ "JOIN timesheet_document_details_new tdd\n"
				+ "ON tdd.timesheet_id = lts.last_timesheet_id\n"
				+ "INNER JOIN final_document_new fdn ON tdd.bulk_approved_doc_id = final_doc_id\n"
				+ "INNER JOIN doc_mime_type_master_new dmm\n"
				+ "ON fdn.mime_type_id = dmm.mime_type_id\n"
				+ "WHERE COALESCE(tdd.final_flag, 0) = 1\n"
				+ ")\n"
				+ "	SELECT DISTINCT bpe.name,bpe.project_id,bpe.project_name,bpe.team_id,bpe.employement_id,lwdd.doc_id,\n"
				+ "	lwdd.mime_type,lwdd.doc_name,lwdd.final_flag ,lwdd.file_url\n"
				+ "	FROM Base_Project_Employees bpe\n"
