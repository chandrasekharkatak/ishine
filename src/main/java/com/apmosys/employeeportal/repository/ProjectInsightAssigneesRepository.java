...
public interface ProjectInsightAssigneesRepository extends JpaRepository<ProjectInsightAssignees, Long> {
	...
	@Query(value="Select distinct e.name from ProjectInsightAssignees pia inner join Employee e on e.empId=pia.assignedTo where pia.entityId=:entityId and pia.entityType=:entityType and (pia.assignType is null  or pia.assignType = 'Assigned' ) ")
	List<String> getUserNameByEntityIdAndEntityType(Long entityId, String entityType);
	
	@Query(value="Select distinct e.empId from ProjectInsightAssignees pia inner join Employee e on e.empId=pia.assignedTo where pia.entityId=:entityId and pia.entityType=:entityType and (pia.assignType is null  or pia.assignType = 'Assigned' ) ")
