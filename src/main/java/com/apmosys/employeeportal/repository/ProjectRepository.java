...
public interface ProjectRepository extends JpaRepository<Project, Integer> {
	...
	@Query(value = "WITH RECURSIVE Date_Generator (dt) AS (\n"
			...
			+ "            c.client_id, c.client_name, p.po_end_date,\n"
			...
			+ "            emp_id,\n"
			+ "            date,\n"
			+ "            office_in_time,office_out_time,client_in_time,client_out_time,\n"
			+ "            COUNT(emp_id) AS submitted_count\n"
			+ "        FROM employee_timesheets\n"
			+ "        WHERE MONTH(date) = MONTH(CURRENT_DATE()) AND YEAR(date) = YEAR(CURRENT_DATE())\n"
			+ "        AND day_type LIKE '%Working%'\n"
			+ "        GROUP BY emp_id ,date,office_in_time,office_out_time,client_in_time,client_out_time\n"
			+ "    ),\n"
			+ "\n"
			+ "    Document_Summary AS (\n"
			+ "        SELECT\n"
			...
			+ "\n"
			+ "SELECT DISTINCT\n"
			+ "    bpe.team_id, bpe.team_name, bpe.emp_id, bpe.name, bpe.employee_role, bpe.billable_type,\n"
			+ "    bpe.start_date, bpe.active, bpe.project_id, bpe.project_name, bpe.client_id, bpe.client_name,\n"
//			+ "    bpe.po_end_date,"
			+ "	   bpe.spoc, bpe.teamLead, bpe.employee_team_map_id, bpe.employement_id,\n"
			+ "    bpe.email, bpe.mobile_no,ts.date,\n"
			+ "    ts.office_in_time,ts.office_out_time,ts.client_in_time,ts.client_out_time,\n"
			+ "    wds.expected_fill_count AS expectedTimesheetFillCount,\n"
			+ "    IFNULL(ts.submitted_count, 0) AS apmosysTimesheetFilledCount,\n"
			...
			+ "LEFT JOIN\n"
			+ "    Timesheet_Summary ts ON bpe.emp_id = ts.emp_id\n"
			+ "LEFT JOIN\n"
			+ "    Document_Summary ds ON bpe.emp_id = ds.emp_id\n"
			+ "LEFT JOIN\n"
			+ "    Project_Manager_Summary pms ON bpe.project_id = pms.project_id\n"
			+ "WHERE 1=1\n"
			+ "      and bpe.project_id IN (:projectId) \n"
			+ "ORDER BY\n"
			+ "    bpe.project_name, bpe.name",nativeQuery = true)
