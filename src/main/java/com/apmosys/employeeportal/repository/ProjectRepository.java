...
public List<Project> findProjectsOfProjectManager(Long projectManagerId);
	...
	@Query(value="select distinct p.project_name,p.po_no,p.project_id\n"
			+ " from projects p\n"
	...
	public List<Object[]> getProjectWithCliendSideID();
	  ...
	  @Query(nativeQuery = true,value ="WITH RECURSIVE\n"
		    ...
		    @Query(value = "WITH RECURSIVE\n" +
		            ...
		            "        FROM employee_timesheets\n" +
		            "        WHERE date BETWEEN (SELECT from_date FROM Date_Parameters) AND (SELECT to_date FROM Date_Parameters)\n" +
		            ...
		            "               COUNT(CASE WHEN client_approval_status = 'approved' THEN 1 END) AS Client_Approved_count\n" +
		            "        FROM timesheet_document_details\n" +
		            "        WHERE DATE(created_on) BETWEEN (SELECT from_date FROM Date_Parameters) AND (SELECT to_date FROM Date_Parameters)\n" +
		            "        GROUP BY emp_id\n" +
		            "    ),\n" +
		            "    Per_Employee_Summary AS (\n" +
		            "        SELECT bpe.emp_id, bpe.project_id, bpe.project_name, pms.Project_Manager, bpe.po_no, bpe.project_type, bpe.client_name,\n" +
		            ...
		            "               CASE\n" +
		            "                   WHEN SUM(ClientSideNotFilledTimesheets_count) > 0 THEN 'Defaulter'\n" +
		            "                   WHEN SUM(ClientSidePendingTimesheet_count) > 0 THEN 'Pending'\n" +
		            "                   ELSE 'Approved'\n" +
		            ...
		            "                 apmosysrm, apmosys_rm_email, clientrm\n" +
		            "    )\n" +
		            "SELECT * FROM Project_Level_Summary \n"
		            + "    WHERE (:status = 'All' OR project_status = :status)",
		            nativeQuery = true)
		public List<Object[]> getProjectViewForClientAttendanceStatus(@Param("month") Integer month,
...
//		public List<Object[]> getProjectViewForClientAttendanceStatus();
	...
	@Query(nativeQuery=true,value="WITH RECURSIVE\n"
			...
			+ "             (SELECT 0 AS a UNION ALL SELECT 10 UNION ALL SELECT 20 UNION ALL SELECT 30 UNION ALL SELECT 40 UNION ALL SELECT 50) b\n"
			...
			+ "        SELECT dt\n"
			+ "        FROM All_Dates_In_Range\n"
			+ "        WHERE dt NOT IN (SELECT DISTINCT date_of_holiday FROM holiday)\n"
			+ "    ),\n"
			+ "\n"
			+ "    WorkingDays_Summary AS (\n"
			+ "        SELECT COUNT(*) AS expected_fill_count\n"
			+ "        FROM Valid_Working_Dates\n"
			+ "    ),\n"
			+ "\n"
			+ "    Base_Project_Employees AS (\n"
			+ "        SELECT DISTINCT\n"
			...
			+ "        left join employee_client_side_id_mapping ecsm on e.emp_id = ecsm.emp_id and ecsm.project_id = p.project_id -- Corrected join from t.project_id\n"
			+ "        left JOIN project_manager_mapping pm on p.project_id = pm.project_id\n"
			+ "		LEFT JOIN employee e1 on e1.emp_id = pm.project_manager_id\n"
			+ "        WHERE etm.active != 0 AND t.is_active != 'N' AND p.active != 'false' AND  e.employmentstatus != 'InActive' AND date(etm.start_date) < curdate()\n"
			+ "    ),\n"
			+ "\n"
			+ "    Timesheet_Summary AS (\n"
			+ "        SELECT emp_id, date, office_in_time, office_out_time, client_in_time, client_out_time\n"
			+ "        FROM employee_timesheets\n"
			+ "        WHERE day_type LIKE '%Working%'\n"
			+ "    ),\n"
			+ "\n"
			+ "    Timesheet_Counts_In_Range AS (\n"
			+ "        SELECT emp_id, COUNT(*) as filled_count\n"
			...
			+ "        SELECT\n"
			+ "            emp_id,\n"
			+ "            GROUP_CONCAT(doc_id SEPARATOR ', ') as docIds,\n"
			+ "            COUNT(CASE WHEN client_approval_status = 'pending' THEN 1 END) AS Client_pending_count,\n"
			+ "            COUNT(CASE WHEN client_approval_status = 'approved' THEN 1 END) AS Client_Approved_count\n"
			...
			+ "    )\n"
			+ "\n"
			+ "SELECT DISTINCT bpe.emp_id,bpe.client_side_id,bpe.start_date,bpe.team_name,bpe.team_id,\n"
			...
			+ "            (wds.expected_fill_count - (IFNULL(ds.Client_pending_count, 0) + IFNULL(ds.Client_Approved_count, 0)))\n"
			+ "        ELSE 0\n"
			...
			+ "\n"
			+ "    bpe.reporting_manager_id,bpe.employement_id\n"
			+ "FROM\n"
			+ "    Base_Project_Employees bpe\n"
			+ "CROSS JOIN\n"
			+ "    Valid_Working_Dates vwd\n"
			+ "CROSS JOIN\n"
			+ "    WorkingDays_Summary wds\n"
			+ "LEFT JOIN\n"
			+ "    Timesheet_Summary ts ON bpe.emp_id = ts.emp_id AND vwd.dt = ts.date\n"
			+ "LEFT JOIN\n"
			+ "    Document_Summary ds ON bpe.emp_id = ds.emp_id\n"
			+ "LEFT JOIN\n"
			+ "    Timesheet_Counts_In_Range tcir ON bpe.emp_id = tcir.emp_id\n"
			+ "WHERE\n"
			+ "     bpe.reporting_manager_id = :empId\n"
			+ "ORDER BY\n"
			+ "    bpe.name, vwd.dt")
