	...
	public List<Object[]> getProjectWithCliendSideID();
	  ...
	  @Query(nativeQuery = true,value ="WITH RECURSIVE\n"
		    ...
		    @Query(value = "WITH RECURSIVE\n"
		    		...
		    		+ "    apmosysrm, apmosys_rm_email, clientrm,\n"
		    		+ "    total_expected_fill_count, total_ishine_filled, total_client_side_not_filled,\n"
		    		+ "    total_client_side_pending, total_client_approved, project_status,\n"
		    		+ "    ClientSideApproved_Percent, ClientSidePending_Percent, NotFilled_Percent\n"
		    		+ "FROM Project_Level_Summary\n"
		    		+ "WHERE \n"
		    		+ "    :status = 'All'\n"
		    		+ "    OR (:status = 'Pending' AND has_pending_flag = 1)\n"
		    		+ "    OR (:status = 'Defaulter' AND has_defaulter_flag = 1)\n"
		    		+ "    OR (:status = 'Approved' AND has_defaulter_flag = 0 AND has_pending_flag = 0)",
		            nativeQuery = true)
...
//		public List<Object[]> getProjectViewForClientAttendanceStatus();
	...
	@Query(nativeQuery=true,value="WITH RECURSIVE\n"
			...
			+ "			    )\n"
			+ "			\n"
			+ "			SELECT DISTINCT bpe.emp_id,case when bpe.has_client_side_id = true then bpe.client_side_id\n"
			+ "            else null end as client_side_id\n"
			+ "            ,\n"
			+ "            \n"
			+ "            bpe.start_date,bpe.team_name,bpe.team_id,\n"
