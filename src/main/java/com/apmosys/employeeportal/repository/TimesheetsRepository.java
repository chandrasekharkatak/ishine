...
//				+ "        LEFT JOIN Holidays_In_Range h ON adir.dt = h.date_of_holiday\n"
	...
	@Query(value="WITH RECURSIVE\n"
			...
			+ "                JOIN project_manager_mapping pm_inner\n"
			+ "                    ON p_inner.project_id = pm_inner.project_id\n"
			...
			+ "                SELECT 1\n"
			+ "                FROM auth_emp p_inner\n"
			+ "                JOIN project_overhead_mapping pom_inner\n"
			+ "                    ON p_inner.project_id = pom_inner.project_id\n"
			+ "                WHERE p_inner.emp_id = e.emp_id\n"
			...
			+ "                AND (etm_inner.spoc_id = :emp_id\n"
			+ "                     OR etm_inner.team_lead_id = :emp_id)\n"
			+ "            )\n"
			+ "        )\n"
			+ "    ),\n"
			+ "\n"
			...
			+ "            AND UPPER(et1.day_type)\n"
			+ "                IN ('LEAVE', 'CLIENT HOLIDAY', 'PUBLIC HOLIDAY', 'WEEK OFF')\n"
			+ "        )\n"
			+ "    ),\n"
			+ "\n"
			+ "    Actual_Client_Side_Submissions AS (\n"
			...
			+ "        FROM Employee_Document_Summary_Details\n"
			+ "        WHERE (UPPER(client_approval_status) = 'APPROVED'\n"
			+ "               OR UPPER(client_approval_status) = 'PENDING')\n"
			+ "          AND timesheet_date < CURDATE()\n"
			+ "    ),\n"
			+ "\n"
			+ "    Combined_Expected_Client_Side_DSR AS (\n"
			+ "        SELECT DISTINCT emp_id, project_id, employee_team_map_id, dt\n"
			+ "        FROM Expected_Client_Side_Base_DSR\n"
			+ "        UNION\n"
			+ "        SELECT DISTINCT emp_id, project_id, employee_team_map_id, dt\n"
			+ "        FROM Actual_Client_Side_Submissions\n"
			+ "    ),\n"
			+ "\n"
			+ "    WorkingDays_Summary AS (\n"
			+ "        SELECT DISTINCT\n"
			...
			+ "                             SELECT 1\n"
			+ "                             FROM timesheet_document_details\n"
			+ "                             WHERE timesheet_id = edsd.timesheet_id\n"
			...
			+ "                )\n"
			+ "                AND (adir.dt <= bpe.end_date OR bpe.end_date IS NULL)\n"
			+ "                THEN 'A'\n"
			+ "\n"
			+ "                WHEN UPPER(global_ts.day_type) LIKE '%WEEK%OFF%' THEN 'WO'\n"
			+ "                WHEN UPPER(global_ts.day_type) LIKE '%PUBLIC HOLIDAY%' THEN 'AH'\n"
			...
			+ "        FROM Base_Project_Employees bpe\n"
			+ "        CROSS JOIN All_Dates_In_Range adir\n"
			+ "        LEFT JOIN employee_timesheets global_ts\n"
			+ "            ON bpe.emp_id = global_ts.emp_id\n"
			+ "            AND adir.dt = global_ts.date\n"
			+ "        LEFT JOIN Timesheet_Base_Data ts_data_relevant\n"
			+ "            ON bpe.emp_id = ts_data_relevant.emp_id\n"
			+ "            AND adir.dt = ts_data_relevant.date\n"
			+ "            AND bpe.employee_team_map_id = ts_data_relevant.employee_team_map_id\n"
			+ "        LEFT JOIN Timesheet_Base_Data ts_data_all_employee\n"
			+ "            ON bpe.emp_id = ts_data_all_employee.emp_id\n"
			+ "            AND adir.dt = ts_data_all_employee.date\n"
			+ "        LEFT JOIN Employee_Document_Summary_Details doc_approved\n"
			+ "            ON ts_data_relevant.timesheet_id = doc_approved.timesheet_id\n"
			+ "            AND UPPER(doc_approved.client_approval_status) = 'APPROVED'\n"
			+ "            AND doc_approved.final_flag = 1\n"
			+ "            AND bpe.employee_team_map_id = doc_approved.employee_team_map_id\n"
			+ "        LEFT JOIN Employee_Document_Summary_Details doc_pending\n"
			+ "            ON ts_data_relevant.timesheet_id = doc_pending.timesheet_id\n"
			+ "            AND UPPER(doc_pending.client_approval_status) = 'PENDING'\n"
			+ "            AND NOT EXISTS (\n"
			+ "                SELECT 1\n"
			+ "                FROM timesheet_document_details\n"
			+ "                WHERE timesheet_id = doc_pending.timesheet_id\n"
			+ "                  AND UPPER(client_approval_status) = 'APPROVED'\n"
			+ "            )\n"
			+ "            AND bpe.employee_team_map_id = doc_pending.employee_team_map_id\n"
			+ "    ),\n"
			...
			+ "                (COALESCE(eds.approved_days, 0)\n"
			+ "                 + COALESCE(eds.pending_days, 0))\n"
			...
			+ "    final_select AS (\n"
			+ "        SELECT DISTINCT\n"
			+ "            COUNT(DISTINCT ecs.employee_team_map_id)\n"
			+ "                AS total_no_of_applicable_employees,\n"
			+ "\n"
			+ "            COUNT(\n"
			+ "                DISTINCT CASE\n"
			+ "                    WHEN ecs.employee_status = 'Approved'\n"
			+ "                    THEN ecs.employee_team_map_id\n"
			+ "                END\n"
			+ "            ) AS total_approved_employees,\n"
			+ "\n"
			+ "            COUNT(\n"
			+ "                DISTINCT CASE\n"
			+ "                    WHEN ecs.employee_status = 'Pending'\n"
			+ "                    THEN ecs.employee_team_map_id\n"
			+ "                END\n"
			...
			+ "            COUNT(\n"
			+ "                DISTINCT CASE\n"
			+ "                    WHEN ecs.employee_status = 'Defaulter'\n"
			+ "                    THEN ecs.employee_team_map_id\n"
			+ "                END\n"
			...
			+ "            ON bpe.employee_team_map_id = dsd.employee_team_map_id\n"
			...
			+ "        LEFT JOIN employee s_emp\n"
			+ "            ON dsd.shadow_emp_id = s_emp.emp_id\n"
			+ "        GROUP BY\n"
			+ "            bpe.emp_id,\n"
			+ "            bpe.project_id,\n"
			+ "            bpe.employee_team_map_id,\n"
	...
	@Query(value=" WITH RECURSIVE\n"
			...
			+ "            WHEN COALESCE(eds.pending_days, 0) > 0 THEN 'Pending'\n"
			+ "            ELSE 'Approved'\n"
			+ "        END AS employee_status\n"
			+ "    FROM Base_Project_Employees bpe\n"
			+ "    LEFT JOIN WorkingDays_Summary wds ON bpe.employee_team_map_id = wds.employee_team_map_id\n"
			...
			+ "    bpe.employmentstatus,bpe.end_date,\n"
			+ "    SUM(CASE WHEN dsd.daily_status = 'DA' THEN 1 ELSE 0 END) AS 'Ready_for_invoicing',\n"
			+ "    bpe.active,ecs.employee_status,bpe.projectActive\n"
			+ "FROM Base_Project_Employees bpe\n"
			+ "JOIN Date_Parameters dp ON 1=1\n"
			+ "LEFT JOIN Daily_Status_Details dsd ON bpe.employee_team_map_id = dsd.employee_team_map_id\n"
