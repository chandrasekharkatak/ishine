...
//	    		Optional<Timesheet> findByEmpIdAndDate(Long empId, Date date);
	...
	@Query(value="WITH RECURSIVE\n"
			...
			+ "                JOIN job_role jr ON e.job_role_id = jr.job_role_id\n"
			+ "                JOIN department d ON jr.dept_id = d.dept_id\n"
			+ "                WHERE e.emp_id = :emp_id AND (jr.employee_role IN ('SuperAdmin') OR d.name IN ('HR', 'Accounts', 'Resource Management Group'))\n"
			+ "            )\n"
			+ "\n"
			+ "            UNION\n"
			+ "\n"
			...
			+ "    )\n"
			+ "    SELECT\n"
			+ "    (SELECT COUNT(project_id) FROM Authorized_Projects) AS total_no_of_applicable_projects,\n"
			...
			+ "FROM\n"
			+ "    Project_Attributes pa\n"
			+ "INNER JOIN\n"
			+ "    Authorized_Projects ap ON pa.project_id = ap.project_id", nativeQuery = true)
