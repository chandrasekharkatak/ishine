...
//	    		Optional<Timesheet> findByEmpIdAndDate(Long empId, Date date);
		...
		@Query(value="WITH RECURSIVE\n"
				...
				+ "              AND (jr.employee_role IN ('SuperAdmin') OR d.name IN ('HR', 'Accounts', 'Resource Management Group'))\n"
				+ "        )\n"
				+ "\n"
				+ "        UNION\n"
				+ "\n"
				...
				+ "                 AND CAST(:month AS UNSIGNED) = MONTH(CURDATE())\n"
				+ "            THEN CURDATE()\n"
				...
				+ "        c.client_name, t.team_id, t.team_name, tl.name AS team_lead_name,\n"
				...
				+ "      AND e.employmentstatus != 'InActive'\n"
				+ "      AND DATE(etm.start_date) <= (SELECT to_date FROM Date_Parameters)\n"
				+ "    GROUP BY e.emp_id, e.name, p.project_id, p.project_name, p.po_no, project_type, c.client_name,\n"
				+ "             t.team_id, t.team_name, team_lead_name, e.billable, e.billable_type,\n"
				...
				+ "                SELECT date_of_holiday \n"
				+ "                FROM holiday \n"
				+ "                WHERE MONTH(date_of_holiday) = :month \n"
				+ "                  AND YEAR(date_of_holiday) = :year\n"
				+ "            )\n"
				+ "            AND DAYOFWEEK(adir.dt) != 1\n"
				+ "            AND NOT (DAYOFWEEK(adir.dt) = 7 AND (DAY(adir.dt) > 7 AND DAY(adir.dt) <= 14))\n"
				+ "            AND NOT (DAYOFWEEK(adir.dt) = 7 AND (DAY(adir.dt) > 21 AND DAY(adir.dt) <= 28))\n"
				+ "        )\n"
				+ "        OR EXISTS (\n"
				+ "            SELECT 1 FROM employee_timesheets et\n"
				+ "            WHERE et.emp_id = be.emp_id\n"
				...
				+ "            WHEN IFNULL(eed.expected_days_passed, 0) > \n"
				+ "                 (IFNULL(eds.approved_days, 0) + IFNULL(eds.pending_days, 0))\n"
				+ "                 OR IFNULL(eds.pending_days, 0) > 0\n"
				+ "            THEN 'Pending'\n"
				+ "            ELSE 'Approved'\n"
	...
	@Query(value="WITH RECURSIVE\n"
			...
			+ "     WHERE (:status = 'All' OR pls.project_status = :status)) AS total_no_of_applicable_projects,\n"
