...
//				+ " AND p.has_client_side_id = 1\n"
		...
		@Query(value=" WITH RECURSIVE\n"
				...
				+ "											upper(client_approval_status) = 'APPROVED' ) ) THEN 'CN'\n"
...
//				+ "        LEFT JOIN Holidays_In_Range h ON adir.dt = h.date_of_holiday\n"
		...
		@Query(value="WITH RECURSIVE\n"
				...
				+ "								        e1.name Project_manager_name,\n"
				+ "								        e.email,\n"
				+ "								        e.mobile_no,e.employmentstatus,etm.end_date\n"
				+ "								    FROM projects p\n"
				+ "								    INNER JOIN teams t ON p.project_id = t.project_id\n"
				+ "								    INNER JOIN employee_team_mapping etm ON t.team_id = etm.team_id\n"
				+ "								    INNER JOIN employee e ON e.emp_id = etm.emp_id\n"
				...
				+ "								    LEFT JOIN project_manager_mapping pm on p.project_id = pm.project_id and pm.active = 1\n"
				+ "								    LEFT JOIN employee e1 on e1.emp_id = pm.project_manager_id\n"
				+ "								    WHERE 1=1 --  etm.active != 0 AND t.is_active != 'N'\n"
				+ "								    -- AND p.active != 'false' \n"
				+ "				                    AND p.has_client_side_id = true\n"
				+ "								    -- AND e.employmentstatus != 'InActive'\n"
				...
				+ "								),\n"
				+ "								Expected_Client_Side_Base_DSR AS (\n"
				+ "								    SELECT distinct bpe.employee_team_map_id,\n"
				+ "								        bpe.emp_id,\n"
				+ "								        bpe.project_id,\n"
				+ "								        adir.dt\n"
				+ "								    FROM Base_Project_Employees bpe\n"
				...
				+ "								\n"
				+ "								Actual_Client_Side_Submissions AS (\n"
				+ "								    SELECT DISTINCT etm.employee_team_map_id\n"
				+ "								        ,et.emp_id\n"
				+ "								        ,p.project_id\n"
				+ "								        ,et.date AS dt\n"
				+ "								    FROM employee_timesheets et\n"
				+ "								                        INNER JOIN employee_team_mapping etm ON etm.emp_id = et.emp_id \n"
				+ "																	AND et.date BETWEEN etm.start_date AND COALESCE(etm.end_date, '2099-12-31')\n"
				+ "								                        INNER JOIN teams t ON etm.team_id = t.team_id\n"
				+ "								                        INNER JOIN projects p ON p.project_id = t.project_id\n"
				+ "								    INNER JOIN timesheet_document_details tdd ON et.timesheet_id = tdd.timesheet_id\n"
				+ "								    JOIN Date_Parameters dp ON et.date BETWEEN dp.from_date AND dp.to_date\n"
				...
				+ "								    SELECT distinct\n"
				+ "								        emp_id,\n"
				+ "								        project_id,employee_team_map_id,\n"
				+ "								        COUNT(DISTINCT dt) AS expected_fill_count\n"
				+ "								    FROM Combined_Expected_Client_Side_DSR\n"
				...
				+ "                                                        inner join Base_Project_Employees bpe on bpe.emp_id = et.emp_id and p.project_id = bpe.project_id and etm.employee_team_map_id = bpe.employee_team_map_id\n"
				+ "								    JOIN Date_Parameters dp ON et.date BETWEEN dp.from_date AND dp.to_date\n"
				+ "								    WHERE 1=1\n"
				+ "								                        -- and etm.active != 0 AND t.is_active != 'N'\n"
				+ "								    -- AND p.active != 'false' \n"
				...
				+ "								        tdd.client_approval_status,\n"
				+ "								        tdd.final_flag,etm.employee_team_map_id,\n"
				+ "								        tdd.active\n"
				+ "								                            ,tdd.timesheet_id\n"
				+ "								    FROM timesheet_document_details tdd\n"
				...
				+ "								        etr.client_out_time,\n"
				+ "								        CASE\n"
				+ "								            WHEN adir.dt < DATE(bpe.start_date) OR bpe.active = 0 OR bpe.active IS NULL OR (bpe.end_date IS NOT NULL AND adir.dt > DATE(bpe.end_date))\n"
				+ "				                            THEN 'O'\n"
				+ "								            WHEN EXISTS (select 1 from employee_timesheets et1 \n"
				+ "															left join  employee_timesheet_activities_mapping etam on et1.timesheet_id = etam.timesheet_id\n"
				+ "								                            left join activities a on etam.activity_id = a.activity_id\n"
				...
				+ "												WHEN EXISTS (SELECT 1 FROM Employee_Document_Summary_Details edsd\n"
				+ "								                        WHERE edsd.emp_id = bpe.emp_id\n"
				+ "								                          AND edsd.project_id = bpe.project_id and edsd.employee_team_map_id = bpe.employee_team_map_id\n"
				+ "								                          AND edsd.timesheet_date = adir.dt\n"
				+ "								                          AND upper(edsd.client_approval_status) = 'APPROVED'\n"
				+ "								                          AND edsd.final_flag = 1) THEN 'CA'\n"
				+ "								            WHEN EXISTS (SELECT 1 FROM Employee_Document_Summary_Details edsd\n"
				...
				+ "				                        SUM(CASE WHEN dsd.daily_status IN ('CA') THEN 1 ELSE 0 END) AS Ready_for_invoicing,\n"
				...
				+ "								    GROUP BY dsd.emp_id, dsd.project_id,dsd.employee_team_map_id\n"
				+ "								)\n"
				+ "								\n"
				+ "								SELECT distinct\n"
				+ "								bpe.emp_id,bpe.client_side_id,bpe.start_date,bpe.team_name,bpe.team_id,\n"
				...
				+ "				                COALESCE(dsc.Ready_for_invoicing,0) as 'Ready_For_invoicing', bpe.active\n"
				+ "								FROM\n"
				+ "								Base_Project_Employees bpe\n"
				+ "								LEFT JOIN\n"
				+ "								Daily_Status_Details dsd ON bpe.emp_id = dsd.emp_id AND bpe.project_id = dsd.project_id\n"
				+ "								LEFT JOIN\n"
				+ "								WorkingDays_Summary wds ON bpe.emp_id = wds.emp_id AND bpe.project_id = wds.project_id and wds.employee_team_map_id = bpe.employee_team_map_id\n"
				+ "								LEFT JOIN\n"
				+ "								Apmosys_Timesheet_Summary ats ON bpe.emp_id = ats.emp_id AND bpe.project_id = ats.project_id\n"
				+ "								LEFT JOIN\n"
				+ "								Employee_Document_Summary eds ON bpe.emp_id = eds.emp_id AND bpe.project_id = eds.project_id\n"
				+ "								LEFT JOIN\n"
				+ "								Daily_Status_Counts dsc ON bpe.emp_id = dsc.emp_id AND bpe.project_id = dsc.project_id and dsc.employee_team_map_id = bpe.employee_team_map_id\n"
				+ "								WHERE bpe.project_id in (:project_id)\n"
				+ "								GROUP BY\n"
				+ "								bpe.emp_id,bpe.client_side_id,bpe.start_date,bpe.team_name,bpe.team_id,\n"
				+ "								bpe.name,bpe.spoc, bpe.billable_type,bpe.employee_role, bpe.dept_name, bpe.project_id,\n"
				...
				+ "								bpe.reporting_manager_id,\n"
				+ "								wds.expected_fill_count, eds.pending_days, eds.approved_days,bpe.employement_id,\n"
				+ "								dsc.Present, dsc.WeekOff, dsc.Holiday, dsc.Leave, dsc.Comp_Off, dsc.NA_Count, dsc.Half_Day,bpe.employmentstatus, bpe.end_date,\n"
				+ "								dsc.Ready_for_invoicing, bpe.active\n"
		...
		@Query(value="WITH RECURSIVE\n"
				...
				+ "							c.client_id, c.client_name, p.po_no,\n"
				+ "							s.name spoc, tl.name teamLead, etm.employee_team_map_id,\n"
				+ "							e.reporting_manager_id,ecsm.client_side_id,e.employmentstatus,\n"
				+ "							CASE\n"
				+ "								WHEN e.is_apmosys_product = 'true' THEN CONCAT('AP-',e.employeement_id)\n"
				+ "								ELSE CONCAT('A-',e.employeement_id)\n"
				+ "							END AS employement_id,\n"
				...
				+ "						COUNT(DISTINCT\n"
				+ "							CASE\n"
				+ "								WHEN upper(et.day_type) IN ('WORKING', 'NON-WORKING') AND a.team_id = brd.team_id THEN et.date\n"
				+ "								WHEN upper(et.day_type) IN ('WEEK OFF', 'PUBLIC HOLIDAY', 'HOLIDAY', 'CLIENT HOLIDAY','LEAVE') THEN et.date\n"
				+ "								ELSE NULL\n"
				+ "							END\n"
				+ "						) AS filled_ishine_days,\n"
				+ "						COUNT(DISTINCT CASE WHEN upper(et.day_type) IN ('WORKING', 'NON-WORKING') AND et.status = 'Pending' AND a.team_id = brd.team_id THEN et.date END) AS ishine_pending_Days,\n"
				+ "						COUNT(DISTINCT CASE WHEN upper(et.day_type) IN ('WORKING', 'NON-WORKING') AND et.status = 'Approved' AND a.team_id = brd.team_id THEN et.date END) AS ishine_approved_Days\n"
				+ "					FROM employee_timesheets et\n"
				+ "					JOIN Date_Parameters dp ON et.date BETWEEN dp.from_date AND dp.to_date\n"
				+ "					LEFT JOIN employee_timesheet_activities_mapping etam ON et.timesheet_id = etam.timesheet_id\n"
				+ "					LEFT JOIN activities a ON a.activity_id = etam.activity_id\n"
				...
				+ "					WHERE\n"
				+ "						(\n"
				+ "							(upper(et.day_type) IN ('WORKING', 'NON-WORKING') AND a.team_id = brd.team_id)\n"
				+ "							OR\n"
				+ "							upper(et.day_type) IN ('WEEK OFF', 'PUBLIC HOLIDAY', 'HOLIDAY', 'CLIENT HOLIDAY','LEAVE')\n"
				+ "						)\n"
				+ "					GROUP BY et.emp_id, brd.project_id \n"
				+ "				),\n"
				+ "				Daily_Status_Details AS (\n"
				...
				+ "											   AND ewd.expected_working_day_date = adir.dt)\n"
				+ "								 AND et.timesheet_id IS NULL THEN 'A'\n"
				+ "							WHEN adir.dt < DATE(brd.start_date) OR (brd.end_date IS NOT NULL AND adir.dt > DATE(brd.end_date)) THEN 'O'\n"
				+ "							ELSE 'NA'\n"
				+ "						END AS daily_status\n"
				+ "					FROM Base_Report_Details brd\n"
				+ "					CROSS JOIN All_Dates_In_Range adir\n"
				...
				+ "					,brd.employmentstatus,brd.end_date,'NA' AS Ready_for_invoicing\n"
				+ "					,brd.active"
				+ "                    FROM\n"
				+ "						Base_Report_Details brd\n"
				+ "					LEFT JOIN\n"
				...
				+ "						Project_Managers_Aggregated pma ON brd.project_id = pma.project_id \n"
				+ "					LEFT JOIN\n"
				+ "						Daily_Status_Counts dsc ON brd.emp_id = dsc.emp_id AND brd.project_id = dsc.project_id\n"
	...
	@Query(value="WITH RECURSIVE\n"
			...
			+ "                auth_emp as\n"
			+ "                (\n"
			+ "                select etm.emp_id as emp_id,p.project_id as project_id,t.spoc_id as spoc_id,t.team_lead_id as team_lead_id,\n"
			+ "                t.team_id as team_id, etm.employee_team_map_id as employee_team_map_id,etm.start_date as start_date,etm.end_date as end_date\n"
			+ "                from employee_team_mapping etm inner join teams t on etm.team_id = t.team_id\n"
			+ "                inner join projects p on t.project_id = p.project_id\n"
			...
			+ "							EXISTS (\n"
			+ "								SELECT 1\n"
			+ "								FROM auth_emp p_inner\n"
			+ "								inner JOIN project_manager_mapping pm_inner ON p_inner.project_id = pm_inner.project_id\n"
			+ "								WHERE p_inner.emp_id = e.emp_id\n"
			+ "								  AND  pm_inner.project_manager_id = 3\n"
			+ "									)\n"
			+ "									 OR\n"
			+ "							EXISTS (\n"
			+ "								SELECT 1\n"
			...
			+ "								 inner JOIN project_overhead_mapping pom_inner ON p_inner.project_id = pom_inner.project_id\n"
			+ "								WHERE p_inner.emp_id = e.emp_id\n"
			+ "								  AND  pom_inner.project_overhead_id = 3\n"
			+ "									)\n"
			+ "									 OR\n"
			+ "							EXISTS (\n"
			+ "								SELECT 1\n"
			+ "								FROM auth_emp etm_inner \n"
			+ "							  WHERE etm_inner.emp_id = e.emp_id\n"
			+ "								  AND  (etm_inner.spoc_id = 3 or etm_inner.team_lead_id = 3)\n"
			+ "									)\n"
			+ "					)),\n"
			+ "				Base_Project_Employees AS (\n"
			+ "					SELECT DISTINCT\n"
			...
			+ "						e.reporting_manager_id,e.employmentstatus,\n"
			+ "						 CASE\n"
			+ "							WHEN e.is_apmosys_product = 'true' THEN CONCAT('AP-',e.employeement_id)\n"
			+ "							ELSE CONCAT('A-',e.employeement_id)\n"
			+ "						END AS employement_id,\n"
			...
			+ "					LEFT JOIN project_manager_mapping pm on p.project_id = pm.project_id -- and pm.active = 1\n"
			+ "					LEFT JOIN employee e1 on e1.emp_id = pm.project_manager_id\n"
			+ "                    inner join auth_emp ea on ea.project_id = p.project_id\n"
			+ "					WHERE 1=1 \n"
			+ "					AND DATE(etm.start_date) <= (SELECT to_date FROM Date_Parameters)\n"
			+ "					AND (etm.end_date IS NULL OR DATE(etm.end_date) >= (SELECT from_date FROM Date_Parameters))\n"
			+ "				),\n"
			+ "				Expected_Client_Side_Base_DSR AS (\n"
			+ "					SELECT distinct\n"
			+ "						bpe.emp_id,bpe.employee_team_map_id,\n"
			+ "						bpe.project_id,\n"
			+ "						adir.dt\n"
			+ "					FROM Base_Project_Employees bpe\n"
			+ "					CROSS JOIN All_Dates_In_Range adir\n"
			...
			+ "					AND NOT EXISTS (\n"
			+ "							   SELECT 1 FROM employee_timesheets et1 where et1.emp_id = bpe.emp_id and adir.dt = et1.date\n"
			+ "							   and(    upper(et1.day_type) like '%LEAVE%'\n"
			...
			+ "						et.date AS dt,\n"
			+ "						et.shadow_emp_id,\n"
			+ "                        ae.employee_team_map_id\n"
			+ "					FROM employee_timesheets et\n"
			+ "                    inner join auth_emp ae on et.emp_id = ae.emp_id  AND et.date BETWEEN ae.start_date AND COALESCE(ae.end_date, '2099-12-31')\n"
			+ "					INNER JOIN timesheet_document_details tdd ON et.timesheet_id = tdd.timesheet_id\n"
			+ "					JOIN Date_Parameters dp ON et.date BETWEEN dp.from_date AND dp.to_date\n"
			+ "					WHERE tdd.active = TRUE\n"
			+ "					  AND (upper(tdd.client_approval_status) = 'APPROVED' OR upper(tdd.client_approval_status) = 'PENDING')\n"
			...
			+ "						tdd.active,\n"
			+ "						et.shadow_emp_id\n"
			+ "                        ,tdd.timesheet_id,ae.employee_team_map_id\n"
			+ "					FROM timesheet_document_details tdd\n"
			+ "					INNER JOIN employee_timesheets et ON tdd.timesheet_id = et.timesheet_id\n"
			+ "                    inner join auth_emp ae on ae.emp_id = et.emp_id AND et.date BETWEEN ae.start_date AND COALESCE(ae.end_date, '2099-12-31')\n"
			+ "					JOIN Date_Parameters dp ON DATE(et.date) BETWEEN dp.from_date AND dp.to_date\n"
			+ "					WHERE 1=1\n"
			+ "					  AND tdd.active = TRUE\n"
			+ "				),\n"
			...
			+ "						etr.shadow_emp_id,\n"
			+ "						CASE\n"
			+ "							WHEN adir.dt < DATE(bpe.start_date) OR bpe.active = 0 OR bpe.active IS NULL OR (bpe.end_date IS NOT NULL AND adir.dt > DATE(bpe.end_date)) then 'O'\n"
			+ "							WHEN EXISTS (select 1 from employee_timesheets et1\n"
			+ "											left join  employee_timesheet_activities_mapping etam on et1.timesheet_id = etam.timesheet_id\n"
			+ "											left join activities a on etam.activity_id = a.activity_id\n"
			+ "											where et1.date = adir.dt and a.team_id != bpe.team_id\n"
			...
			+ "						END AS daily_status\n"
			+ "					FROM Base_Project_Employees bpe\n"
			+ "                    inner join auth_emp ea on ea.project_id = bpe.project_id\n"
			+ "					CROSS JOIN All_Dates_In_Range adir\n"
			+ "					LEFT JOIN Employee_Timesheets_In_Range etr ON bpe.emp_id = etr.emp_id AND adir.dt = etr.date\n"
			+ "				),\n"
			+ "				Apmosys_Timesheet_Summary AS (\n"
			+ "					SELECT distinct et.emp_id, ea.project_id,ea.employee_team_map_id, COUNT(DISTINCT et.date) AS filled_working_days\n"
			+ "					FROM employee_timesheets et\n"
			+ "                    inner join auth_emp ea on et.emp_id = et.emp_id AND et.date BETWEEN ae.start_date AND COALESCE(ae.end_date, '2099-12-31')\n"
			+ "					JOIN Date_Parameters dp ON et.date BETWEEN dp.from_date AND dp.to_date\n"
			+ "					WHERE (et.day_type LIKE '%Working%' OR upper(et.day_type) like '%LEAVE%')\n"
			+ "					GROUP BY et.emp_id, ea.project_id,ea.employee_team_map_id\n"
			+ "				),\n"
			+ "				Employee_Document_Summary AS (\n"
			+ "					SELECT distinct\n"
			+ "						edsd.emp_id,\n"
			+ "						edsd.project_id,\n"
			+ "                        ea.employee_team_map_id,\n"
			+ "					   COUNT(DISTINCT CASE WHEN upper(edsd.client_approval_status) = 'APPROVED' AND final_flag = 1 THEN edsd.timesheet_id END) AS approved_days,\n"
			+ "						   COUNT(DISTINCT CASE WHEN upper(edsd.client_approval_status) = 'PENDING' AND edsd.timesheet_id\n"
			+ "													not in (select timesheet_id from timesheet_document_details where\n"
			+ "											upper(client_approval_status) = 'APPROVED' ) THEN edsd.timesheet_id END) AS pending_days\n"
			+ "					FROM Employee_Document_Summary_Details edsd\n"
			+ "                    inner join auth_emp ea on ea.project_id = edsd.project_id AND edsd.timesheet_date BETWEEN ea.start_date AND COALESCE(ea.end_date, '2099-12-31')\n"
			+ "					GROUP BY edsd.emp_id, edsd.project_id,ea.employee_team_map_id\n"
			+ "				)\n"
			+ "				, Daily_Status_Counts AS (\n"
			+ "					SELECT distinct\n"
			+ "						dsd.emp_id,\n"
			...
			+ "						SUM(CASE WHEN dsd.daily_status IN ('DA') THEN 1 ELSE 0 END) AS Ready_for_invoicing,\n"
			+ "						SUM(CASE WHEN dsd.daily_status = 'WO' THEN 1 ELSE 0 END) AS WeekOff,\n"
			+ "						SUM(CASE WHEN dsd.daily_status IN ('AH', 'CH') THEN 1 ELSE 0 END) AS Holiday,\n"
			+ "						SUM(CASE WHEN dsd.daily_status = 'L' THEN 1 ELSE 0 END) AS 'Leave',\n"
			+ "						SUM(CASE WHEN dsd.daily_status in ('A','O') THEN 1 ELSE 0 END) AS NA_Count,\n"
			...
			+ "				bpe.emp_id,bpe.client_side_id,bpe.start_date,bpe.team_name,bpe.team_id,\n"
			+ "				CASE\n"
			+ "					WHEN bpe.billable_type = 'Shadow' AND s_emp.name IS NOT NULL THEN CONCAT(bpe.name, ' (Shadow for ', s_emp.name, ')')\n"
			+ "					ELSE bpe.name\n"
			+ "				END AS name,\n"
			...
			+ "	MAX(CASE WHEN DAY(dsd.timesheet_date) = 31 THEN dsd.client_out_time END) AS `31_client_out_time`,\n"
			+ "bpe.employement_id,\n"
			+ "	COALESCE(SUM(dsc.Present), 0) AS 'Present',\n"
			+ "	COALESCE(SUM(dsc.WeekOff), 0) AS 'WeekOff',\n"
			+ "	COALESCE(SUM(dsc.Holiday), 0) AS 'Holiday',\n"
			+ "	COALESCE(SUM(dsc.Leave), 0) AS 'Leave',\n"
			+ "	COALESCE(SUM(dsc.NA_Count), 0) AS 'NA_Count',\n"
			+ "	COALESCE(SUM(dsc.Comp_Off), 0) AS 'Comp_Off',\n"
			+ "	COALESCE(SUM(dsc.Half_Day), 0) AS 'Half_Day',\n"
			+ "    (COALESCE(dsc.Present, 0) + COALESCE(dsc.WeekOff, 0) + COALESCE(dsc.Holiday, 0) + COALESCE(dsc.Leave, 0) + COALESCE(dsc.Comp_Off, 0) + COALESCE(dsc.NA_Count, 0) + COALESCE(dsc.Half_Day, 0)) AS total_days,\n"
			+ "	(COALESCE(eds.approved_days, 0) + COALESCE(eds.pending_days, 0)) as ishine_filled_days,\n"
			+ "	bpe.employmentstatus,bpe.end_date\n"
			+ " ,COALESCE(SUM(dsc.Ready_for_invoicing), 0) 'AS Ready_for_invoicing',bpe.active \n"
			+ "	FROM Base_Project_Employees bpe\n"
			+ "	LEFT JOIN Employee_Document_Summary eds ON bpe.emp_id = eds.emp_id AND bpe.project_id = eds.project_id\n"
			+ "	LEFT JOIN WorkingDays_Summary wds ON bpe.emp_id = wds.emp_id AND bpe.project_id = wds.project_id and wds.employee_team_map_id = bpe.employee_team_map_id\n"
			+ "	LEFT JOIN Daily_Status_Details dsd ON bpe.emp_id = dsd.emp_id AND bpe.project_id = dsd.project_id and dsd.employee_team_map_id = bpe.employee_team_map_id\n"
			+ "	LEFT JOIN Daily_Status_Counts dsc ON bpe.emp_id = dsc.emp_id AND bpe.project_id = dsc.project_id and dsc.employee_team_map_id = bpe.employee_team_map_id\n"
			+ "	LEFT JOIN employee s_emp ON dsd.shadow_emp_id = s_emp.emp_id\n"
			+ "    inner join auth_emp ea on ea.project_id = bpe.project_id\n"
			+ "	GROUP BY bpe.emp_id, bpe.project_id, bpe.client_side_id, bpe.name, bpe.billable_type, bpe.spoc,bpe.employee_role, bpe.dept_name, \n"
			+ "             bpe.project_name, bpe.po_no, bpe.client_name,bpe.reporting_manager_id,wds.expected_fill_count, eds.pending_days,\n"
			+ "             eds.approved_days,bpe.start_date,bpe.team_name,bpe.team_id,s_emp.name,dsc.Ready_for_invoicing,bpe.end_date,bpe.employmentstatus\n"
