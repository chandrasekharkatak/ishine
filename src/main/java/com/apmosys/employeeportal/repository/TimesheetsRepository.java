...
//	    		Optional<Timesheet> findByEmpIdAndDate(Long empId, Date date);
		...
		+ "                WHEN tdd_pending.emp_id IS NOT NULL THEN 'DP'\n"
		+ "                WHEN ts.emp_id IS NOT NULL THEN 'P'\n"
		+ "                WHEN adir.dt IN (SELECT dt FROM Valid_Working_Dates) THEN 'A'\n"
		+ "                ELSE 'NA'\n"
		+ "            END AS daily_status\n"
		...
		+ "        LEFT JOIN holiday h ON adir.dt = h.date_of_holiday\n"
		+ "        LEFT JOIN employee_timesheets ts ON bpe.emp_id = ts.emp_id AND adir.dt = ts.date\n"
		+ "        LEFT JOIN timesheet_document_details tdd_approved ON ts.timesheet_id = tdd_approved.timesheet_id AND tdd_approved.client_approval_status = 'approved'\n"
		+ "        LEFT JOIN timesheet_document_details tdd_pending ON ts.timesheet_id = tdd_pending.timesheet_id AND tdd_pending.client_approval_status = 'pending'\n"
		...
		+ "    Document_Summary AS (\n"
		+ "        SELECT emp_id,\n"
		+ "            COUNT(CASE WHEN client_approval_status = 'pending' and tdd.final_flag = 1 THEN 1 END) AS Client_pending_count,\n"
		+ "            COUNT(CASE WHEN client_approval_status = 'approved' and tdd.final_flag = 1 THEN 1 END) AS Client_Approved_count\n"
		+ "        FROM timesheet_document_details tdd JOIN Date_Parameters dp ON DATE(tdd.created_on)\n"
		+ "        BETWEEN dp.from_date AND dp.to_date WHERE active = true GROUP BY emp_id\n"
		+ "    )\n"
		+ "\n"
		+ "SELECT\n"
		+ "    bpe.emp_id,bpe.client_side_id,bpe.start_date,bpe.team_name,bpe.team_id,\n"
		...
		+ "    Document_Summary ds ON bpe.emp_id = ds.emp_id\n"
		+ "WHERE\n"
		+ "     bpe.project_id = (:projectId)\n"
		+ "GROUP BY\n"
		+ "    bpe.emp_id,bpe.client_side_id,bpe.start_date,bpe.team_name,bpe.team_id,\n"
		+ "    bpe.name,bpe.spoc, bpe.billable_type,bpe.employee_role, bpe.dept_name, bpe.project_id,\n"
		+ "    bpe.project_name,bpe.Project_manager_name,bpe.po_no,bpe.client_name,\n"
		+ "    bpe.reporting_manager_id,\n"
		+ "    wds.expected_fill_count, tcir.filled_count, ds.Client_pending_count, ds.Client_Approved_count\n"
		+ "ORDER BY\n"
		+ "    bpe.name" , nativeQuery = true)
	...
	@Query(value="WITH RECURSIVE\n"
			...
			+ "            LAST_DAY(STR_TO_DATE(CONCAT(:year, '-', :month, '-01'), '%Y-%m-%d')) AS to_date\n"
			+ "    ),\n"
			+ "\n"
			+ "    All_Dates_In_Month(dt) AS (\n"
			+ "        SELECT from_date FROM Date_Parameters\n"
			+ "        UNION ALL\n"
			+ "        SELECT DATE_ADD(dt, INTERVAL 1 DAY) FROM All_Dates_In_Month\n"
			...
			+ "        WHERE \n"
			+ "            dt NOT IN (SELECT h.date_of_holiday FROM holiday h JOIN Date_Parameters dp ON h.date_of_holiday BETWEEN dp.from_date AND dp.to_date)\n"
			+ "            AND DAYOFWEEK(dt) != 1 -- Exclude Sunday\n"
			...
			+ "    ),\n"
			+ "\n"
			+ "    Employee_Final_Status AS (\n"
			+ "        SELECT\n"
			+ "            be.emp_id,\n"
			+ "            CASE\n"
			+ "                WHEN dl.emp_id IS NOT NULL THEN 'Defaulter'\n"
			+ "                \n"
			+ "                WHEN IFNULL(eds.pending_days, 0) > 0 OR (IFNULL(eds.approved_days, 0) + IFNULL(eds.pending_days, 0)) < eed.expected_days_passed THEN 'Pending'\n"
			+ "                \n"
			+ "                ELSE 'Compliant'\n"
			+ "            END AS final_status\n"
			+ "        FROM\n"
			...
			+ "            WHERE dt < CURDATE()\n"
			+ "            GROUP BY emp_id\n"
			+ "        ) eed ON be.emp_id = eed.emp_id\n"
			+ "        LEFT JOIN Employee_Document_Summary eds ON be.emp_id = eds.emp_id\n"
			+ "        LEFT JOIN Defaulter_List dl ON be.emp_id = dl.emp_id\n"
			+ "    )\n"
			+ "\n"
			+ "SELECT\n"
			+ "    COUNT(*) AS total_applicable_employees,\n"
			+ "    COUNT(CASE WHEN final_status = 'Compliant' THEN 1 END) AS approved_count,\n"
			+ "    COUNT(CASE WHEN final_status = 'Defaulter' THEN 1 END) AS defaulter_count,\n"
			+ "    COUNT(CASE WHEN final_status = 'Pending' THEN 1 END) AS client_side_pending_count\n"
			+ "FROM\n"
			+ "    Employee_Final_Status", nativeQuery = true)
	...
	@Query(value="WITH RECURSIVE\n"
			...
			+ "        FROM All_Dates_In_Month\n"
			+ "        WHERE \n"
			+ "            dt NOT IN (SELECT date_of_holiday FROM holiday h JOIN Date_Parameters dp ON h.date_of_holiday BETWEEN dp.from_date AND dp.to_date)\n"
			+ "            AND DAYOFWEEK(dt) != 1 \n"
			+ "            AND NOT (DAYOFWEEK(dt) = 7 AND (DAY(dt) > 7 AND DAY(dt) <= 14)) \n"
			+ "            AND NOT (DAYOFWEEK(dt) = 7 AND (DAY(dt) > 21 AND DAY(dt) <= 28))\n"
			+ "    ),\n"
			+ "\n"
			+ "    Base_Employees_In_Projects AS (\n"
			...
			+ "    ),\n"
			+ "\n"
			+ "    Employee_Absent_Days AS (\n"
			+ "        SELECT\n"
			+ "            bep.emp_id,\n"
			+ "            bep.project_id,\n"
			+ "            bwd.dt AS absent_date\n"
			+ "        FROM Base_Employees_In_Projects bep\n"
			+ "        CROSS JOIN Base_Working_Days bwd\n"
			+ "        WHERE \n"
			+ "            bwd.dt < CURDATE()\n"
			+ "            AND NOT EXISTS (\n"
			+ "                SELECT 1 FROM employee_timesheets et\n"
			+ "                WHERE et.emp_id = bep.emp_id AND et.date = bwd.dt AND (et.day_type LIKE '%Leave%' OR et.day_type LIKE '%Client Holiday%')\n"
			+ "            )\n"
			+ "            AND NOT EXISTS (\n"
			+ "                SELECT 1 FROM employee_timesheets et\n"
			+ "                WHERE et.emp_id = bep.emp_id AND et.date = bwd.dt AND et.day_type LIKE '%Working%'\n"
			+ "            )\n"
			+ "    ),\n"
			+ "    \n"
			+ "    Consecutive_Absence_Groups AS (\n"
			+ "        SELECT\n"
			+ "            emp_id, project_id,\n"
			+ "            DATE_SUB(absent_date, INTERVAL ROW_NUMBER() OVER (PARTITION BY emp_id, project_id ORDER BY absent_date) DAY) as consecutive_group_key\n"
			+ "        FROM Employee_Absent_Days\n"
			+ "    ),\n"
			+ "\n"
			+ "    Defaulter_Employees AS (\n"
			+ "        SELECT DISTINCT emp_id, project_id\n"
			+ "        FROM (\n"
			+ "            SELECT emp_id, project_id, COUNT(*) as consecutive_days\n"
			+ "            FROM Consecutive_Absence_Groups\n"
			+ "            GROUP BY emp_id, project_id, consecutive_group_key\n"
			+ "        ) AS Absence_Counts\n"
			...
			+ "    ),\n"
			+ "\n"
			+ "    Employee_Project_Status AS (\n"
			+ "        SELECT\n"
			+ "            bep.project_id,\n"
			+ "            bep.emp_id,\n"
			+ "            CASE\n"
			+ "                WHEN de.emp_id IS NOT NULL THEN 'Defaulter'\n"
			+ "                WHEN IFNULL(eds.pending_days, 0) > 0 OR (IFNULL(eds.approved_days, 0) + IFNULL(eds.pending_days, 0)) < ewdp.expected_days_passed THEN 'Pending'\n"
			+ "                ELSE 'Approved'\n"
			+ "            END AS employee_status\n"
			+ "        FROM Base_Employees_In_Projects bep\n"
			+ "        LEFT JOIN Defaulter_Employees de ON bep.emp_id = de.emp_id AND bep.project_id = de.project_id\n"
			+ "        LEFT JOIN Employee_Document_Summary eds ON bep.emp_id = eds.emp_id\n"
			+ "        LEFT JOIN (\n"
			+ "            SELECT ead.emp_id, COUNT(ead.dt) as expected_days_passed\n"
			+ "            FROM (\n"
			+ "                SELECT be.emp_id, bwd.dt\n"
			+ "                FROM Base_Employees_In_Projects be\n"
			+ "                CROSS JOIN Base_Working_Days bwd\n"
			+ "                WHERE NOT EXISTS (\n"
			+ "                    SELECT 1 FROM employee_timesheets et\n"
			...
			+ "            ) ead\n"
			+ "            GROUP BY ead.emp_id\n"
			+ "        ) ewdp ON bep.emp_id = ewdp.emp_id\n"
			+ "    ),\n"
			+ "\n"
			+ "    Project_Status_Flags AS (\n"
			+ "        SELECT\n"
			+ "            project_id,\n"
			...
			+ "            CASE WHEN MAX(CASE WHEN employee_status != 'Approved' THEN 1 ELSE 0 END) = 0 THEN 1 ELSE 0 END AS is_fully_approved\n"
			+ "        FROM Employee_Project_Status\n"
			+ "        GROUP BY project_id\n"
			+ "    )\n"
			+ "\n"
			+ "SELECT\n"
			+ "    (SELECT COUNT(DISTINCT project_id) FROM Base_Employees_In_Projects) AS total_no_of_applicable_projects,\n"
			+ "    SUM(is_fully_approved) AS total_approved_projects,\n"
			+ "    SUM(has_pending) AS total_pending_projects,\n"
			+ "    SUM(has_defaulter) AS total_defaulter_projects\n"
			+ "FROM\n"
