...
+ ")\n"
	...
	@Query(value= " WITH RECURSIVE\n"
		...
		@Query(value="WITH RECURSIVE\n"
				...
				+ "        ) AS from_date,\n"
				+ "        CASE\n"
				+ "            WHEN 2025 IS NOT NULL AND 11 IS NOT NULL THEN\n"
				+ "                IF(\n"
				+ "                    2025 = YEAR(CURDATE()) AND 11 = MONTH(CURDATE()),\n"
				+ "                    CURDATE(),\n"
				+ "                    LAST_DAY(STR_TO_DATE(CONCAT(2025, '-', 11, '-01'), '%Y-%m-%d'))\n"
				+ "                )\n"
				+ "            ELSE CURDATE()\n"
				+ "        END AS to_date\n"
				+ "),\n"
				...
				+ "      AND (\n"
				+ "            e.date_of_relieving IS NULL\n"
				+ "            OR YEAR(e.date_of_relieving) > 2025\n"
				+ "            OR (YEAR(e.date_of_relieving) = 2025 AND MONTH(e.date_of_relieving) >= 11)\n"
				+ "      )\n"
				+ "      AND e.emp_id NOT BETWEEN 1 AND 6\n"
				+ "      AND DATE(etm.start_date) <= (SELECT to_date FROM Date_Parameters)\n"
				+ "      AND (\n"
