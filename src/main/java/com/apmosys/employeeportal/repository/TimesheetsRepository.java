...
//	    		Optional<Timesheet> findByEmpIdAndDate(Long empId, Date date);
		...
		@Query(value=" WITH RECURSIVE\n"
				...
				+ "                WHEN EXISTS (SELECT 1 FROM Combined_Expected_Client_Side_DSR ecds WHERE ecds.emp_id = bpe.emp_id AND ecds.project_id = bpe.project_id AND ecds.dt = adir.dt)\n"
				+ "                     AND NOT EXISTS (SELECT 1 FROM Employee_Document_Summary_Details edsd_check\n"
				+ "                                     WHERE edsd_check.emp_id = bpe.emp_id\n"
				+ "                                       AND edsd_check.project_id = bpe.project_id\n"
				+ "                                       AND edsd_check.timesheet_date = adir.dt\n"
				+ "                                       AND (upper(edsd_check.client_approval_status) = 'APPROVED' OR upper(edsd_check.client_approval_status) = 'PENDING'))\n"
				+ "                     AND adir.dt < CURDATE() THEN 'M'\n"
				+ "                ELSE 'NA'\n"
				+ "            END AS daily_status\n"
				+ "        FROM Base_Project_Employees bpe\n"
				+ "        CROSS JOIN All_Dates_In_Range adir\n"
...
//				+ "          )\n"
				...
				+ "                WHEN EXISTS (SELECT 1 FROM holiday h WHERE h.date_of_holiday = adir.dt) THEN 'H'\n"
				+ "                WHEN DAYOFWEEK(adir.dt) = 1 THEN 'WO'\n"
				+ "                WHEN DAYOFWEEK(adir.dt) = 7 AND (DAY(adir.dt) BETWEEN 8 AND 14 OR DAY(adir.dt) BETWEEN 22 AND 28) THEN 'WO'\n"
				+ "                WHEN et.timesheet_id IS NOT NULL AND upper(et.day_type) LIKE '%WORKING%' THEN 'P'\n"
				+ "                WHEN adir.dt < CURDATE() AND EXISTS (SELECT 1 FROM Expected_Client_Side_DSR ecdsr WHERE ecdsr.emp_id = bpe.emp_id AND ecdsr.dt = adir.dt) THEN 'A'\n"
				+ "                ELSE 'NA'\n"
				+ "            END AS daily_status\n"
				+ "        FROM Base_Project_Employees bpe\n"
				+ "        CROSS JOIN All_Dates_In_Range adir\n"
