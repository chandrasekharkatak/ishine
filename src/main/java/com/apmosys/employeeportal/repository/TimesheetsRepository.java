...
//	    		Optional<Timesheet> findByEmpIdAndDate(Long empId, Date date);
		...
		@Query(value="WITH RECURSIVE\n"
				...
				+ "                  (DAYOFWEEK(adir.dt) = 7 AND (DAY(adir.dt) BETWEEN 8 AND 14 OR DAY(adir.dt) BETWEEN 22 AND 28))) -- 2nd or 4th Saturday\n"
				+ "            THEN 'W'\n"
				+ "            WHEN adir.dt < DATE(bpe.start_date) THEN 'O'\n"
				+ "            WHEN ts.day_type like '%Leave%' THEN 'A'\n"
				+ "            WHEN ts.day_type like '%Non-Working%' THEN 'NW'\n"
				+ "            WHEN ts.day_type like '%Public Holiday%' THEN 'AH'\n"
				+ "            WHEN ts.day_type like '%Week Off%' THEN 'W'\n"
				...
				+ "ORDER BY\n"
				+ "    bpe.name" , nativeQuery = true)
		public List<Object[]> getEmployeeTimesheetAsCalenderByEmpId(
	...
	@Query(value="WITH RECURSIVE\n"
			...
			+ "        WHERE \n"
			+ "            dt NOT IN (SELECT date_of_holiday FROM holiday WHERE MONTH(date_of_holiday) = 07 AND YEAR(date_of_holiday) = 2025)\n"
			+ "            AND DAYOFWEEK(dt) != 1\n"
			...
			+ "    ),\n"
			+ "    Submitted_Dates AS (\n"
			+ "        SELECT DISTINCT emp_id, DATE(created_on) AS submission_date\n"
			+ "        FROM timesheet_document_details\n"
			+ "        WHERE MONTH(created_on) = 07 AND YEAR(created_on) = 2025 AND active = true\n"
			+ "    ),\n"
			+ "    Missing_Days AS (\n"
			+ "        SELECT\n"
			+ "            awd.emp_id,\n"
			...
			+ "        LEFT JOIN Submitted_Dates sd ON awd.emp_id = sd.emp_id AND awd.dt = sd.submission_date\n"
			...
			+ "        SELECT\n"
			+ "            emp_id,\n"
			+ "            COUNT(CASE WHEN client_approval_status = 'approved' THEN 1 END) AS approved_count,\n"
			+ "            COUNT(CASE WHEN client_approval_status = 'pending' THEN 1 END) AS pending_count\n"
			+ "        FROM timesheet_document_details\n"
			+ "        WHERE MONTH(created_on) = 07 AND YEAR(created_on) = 2025 AND active = true\n"
			+ "        GROUP BY emp_id\n"
			+ "    ),\n"
			+ "    Employee_Expected_Days AS (\n"
			+ "        SELECT emp_id, COUNT(dt) as expected_days\n"
			...
			+ "                WHEN IFNULL(eds.approved_count, 0) >= eed.expected_days THEN 'Compliant'\n"
			+ "                WHEN (IFNULL(eds.approved_count, 0) + IFNULL(eds.pending_count, 0)) >= eed.expected_days THEN 'Pending'\n"
			+ "                ELSE 'Defaulter'\n"
			+ "            END AS final_status\n"
			+ "        FROM\n"
			+ "            Base_Employees be\n"
			+ "        LEFT JOIN\n"
			+ "            Employee_Expected_Days eed ON be.emp_id = eed.emp_id\n"
			+ "        LEFT JOIN\n"
			+ "            Employee_Document_Summary eds ON be.emp_id = eds.emp_id\n"
			+ "        LEFT JOIN\n"
			+ "            Defaulter_List dl ON be.emp_id = dl.emp_id\n"
			+ "    )\n"
			+ "\n"
			+ "SELECT\n"
	...
	@Query(value="WITH RECURSIVE\n"
			...
			+ "    ),\n"
			+ "    Employee_Actual_Working_Days AS (\n"
			+ "        SELECT be.emp_id, be.project_id, COUNT(bwd.dt) as expected_days\n"
			+ "        FROM Base_Employees_In_Projects be\n"
			+ "        JOIN Base_Working_Days bwd ON 1=1\n"
			+ "        WHERE NOT EXISTS (\n"
			+ "            SELECT 1 FROM employee_timesheets et\n"
			+ "            WHERE et.emp_id = be.emp_id\n"
			+ "              AND et.date = bwd.dt\n"
			...
			+ "            eawd.emp_id,\n"
			+ "            CASE\n"
			+ "                WHEN (IFNULL(eds.approved_count, 0) + IFNULL(eds.pending_count, 0)) < eawd.expected_days THEN 'Defaulter'\n"
			+ "                WHEN IFNULL(eds.pending_count, 0) > 0 THEN 'Pending'\n"
			+ "                ELSE 'Compliant'\n"
			+ "            END AS employee_status\n"
			+ "        FROM Employee_Actual_Working_Days eawd\n"
			+ "        LEFT JOIN Employee_Document_Summary eds ON eawd.emp_id = eds.emp_id\n"
			+ "    ),\n"
			+ "    Project_Final_Status AS (\n"
			+ "        SELECT\n"
			+ "            project_id,\n"
			+ "            CASE\n"
			...
			+ "    COUNT(CASE WHEN final_project_status = 'Approved' THEN 1 END) AS total_approved_projects,\n"
			+ "    COUNT(CASE WHEN final_project_status = 'Pending' THEN 1 END) AS total_pending_projects,\n"
			+ "    COUNT(CASE WHEN final_project_status = 'Defaulter' THEN 1 END) AS total_defaulter_projects\n"
			+ "FROM\n"
			+ "    Project_Final_Status", nativeQuery = true)
