...
//	    		Optional<Timesheet> findByEmpIdAndDate(Long empId, Date date);
		...
		@Query(value=" WITH RECURSIVE\n"
				...
				+ "                    WHERE et.emp_id = bpe.emp_id AND et.date = adir.dt\n"
				+ "                      AND et.day_type LIKE '%Working%' AND tdd.active = TRUE \n"
				+ "                )\n"
				+ "            )\n"
				+ "    ),\n"
				+ "\n"
				...
				+ "                WHEN ts.emp_id IS NOT NULL THEN 'P' \n"
				+ "                WHEN adir.dt < CURDATE() AND adir.dt IN (SELECT dt FROM Dynamic_Expected_Days WHERE emp_id = bpe.emp_id) THEN 'A' \n"
				+ "                ELSE 'NA'\n"
				+ "            END AS daily_status\n"
				+ "        FROM Base_Project_Employees bpe\n"
				+ "        CROSS JOIN All_Dates_In_Range adir\n"
				...
				+ "    Document_Summary AS (\n"
				+ "        SELECT tdd.emp_id,\n"
				+ "            COUNT(DISTINCT CASE WHEN upper(tdd.client_approval_status) = 'PENDING' AND tdd.final_flag = 1\n"
				...
				+ "    Base_Project_Employees bpe\n"
				+ "LEFT JOIN\n"
				+ "    Daily_Status_Details dsd ON bpe.emp_id = dsd.emp_id AND dsd.timesheet_date BETWEEN (SELECT from_date FROM Date_Parameters) AND (SELECT to_date FROM Date_Parameters) -- Added date filter for DSD join\n"
				+ "LEFT JOIN\n"
		...
		@Query(value="WITH RECURSIVE\n"
				...
				+ "			             p.clientrm, employement_id, dept_name\n"
				+ "			),\n"
				+ "			\n"
				...
				+ "			    SELECT DISTINCT emp_id\n"
				+ "			    FROM (\n"
				+ "			        SELECT emp_id, grp, COUNT(*) AS streak_length\n"
				+ "			        FROM Missing_Day_Groups_CTE\n"
				+ "			        GROUP BY emp_id, grp\n"
				+ "			        HAVING COUNT(*) > 2\n"
				+ "			    ) AS streaks\n"
				+ "			),\n"
				...
				+ "			        GREATEST(0, COALESCE(eed.expected_days_passed, 0) - \n"
				...
				+ "			            WHEN COALESCE(eed.expected_days_passed, 0) > \n"
	...
	@Query(value="WITH RECURSIVE\n"
			...
			+ "            IFNULL(atfd.filled_working_days, 0) AS ishine_timesheet_filled_count,\n"
			+ "            GREATEST(0, IFNULL(eed.expected_days_passed, 0) - (IFNULL(ds.Client_Approved_count, 0) + IFNULL(ds.Client_pending_count, 0))) AS ClientSideNotFilledTimesheets_count,\n"
			...
			+ "                WHEN IFNULL(eed.expected_days_passed, 0) > (IFNULL(ds.Client_Approved_count, 0) + IFNULL(ds.Client_pending_count, 0)) OR IFNULL(ds.Client_pending_count, 0) > 0 THEN 'Pending'\n"
			+ "                ELSE 'Approved'\n"
			+ "            END AS employee_status\n"
			+ "        FROM Base_Project_Employees bpe\n"
			+ "        LEFT JOIN Document_Summary ds ON bpe.emp_id = ds.emp_id\n"
			+ "        LEFT JOIN Defaulter_Employees de ON bpe.emp_id = de.emp_id\n"
			+ "        LEFT JOIN Apmosys_Timesheet_Filled_Days atfd ON bpe.emp_id = atfd.emp_id\n"
			+ "        LEFT JOIN (\n"
			+ "            SELECT emp_id, project_id, COUNT(dt) as expected_days_passed\n"
			+ "            FROM Employee_Actual_Working_Days\n"
			+ "            GROUP BY emp_id, project_id\n"
			+ "        ) eed ON bpe.emp_id = eed.emp_id AND bpe.project_id = eed.project_id\n"
			+ "    ),\n"
			...
			+ "                    FROM Base_Project_Employees bpe\n"
			+ "                    WHERE EXISTS (\n"
			+ "                        SELECT 1 FROM employee e_auth\n"
			+ "                        JOIN job_role jr ON e_auth.job_role_id = jr.job_role_id\n"
			+ "                        JOIN department d ON jr.dept_id = d.dept_id\n"
			+ "                        WHERE e_auth.emp_id = :emp_id AND (jr.employee_role IN ('SuperAdmin') OR d.name IN ('HR', 'Accounts', 'Resource Management Group'))\n"
			+ "                    )\n"
			+ "\n"
			+ "                    UNION\n"
			+ "\n"
			...
			+ "            )\n"
			+ "SELECT\n"
			+ "    (SELECT COUNT(ap.project_id) FROM Authorized_Projects ap\n"
			+ "     INNER JOIN Project_Level_Summary pls ON ap.project_id = pls.project_id\n"
			+ "     ) AS total_no_of_applicable_projects,\n"
