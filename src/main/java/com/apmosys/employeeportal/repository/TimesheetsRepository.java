...
+ ")\n"
	...
	@Query(value= " WITH RECURSIVE\n"
	    ...
	    @Query(value = "WITH RECURSIVE\n"
	    		...
	    		+ "               COUNT(DISTINCT CASE WHEN UPPER(client_approval_status) = 'APPROVED' AND UPPER(status) != 'REJECTED' AND final_flag = 1 THEN timesheet_id END) AS approved_days,\n"
	    		+ "               COUNT(DISTINCT CASE WHEN UPPER(client_approval_status) = 'PENDING' AND UPPER(status) != 'REJECTED' AND NOT EXISTS (SELECT 1 FROM timesheet_document_details WHERE timesheet_id = edsd.timesheet_id AND UPPER(client_approval_status) = 'APPROVED') THEN timesheet_id END) AS pending_days\n"
	    		...
	    		+ "        SELECT emp_id, employee_team_map_id,\n"
	    		+ "               COUNT(DISTINCT CASE WHEN UPPER(client_approval_status) = 'APPROVED' AND final_flag = 1 AND UPPER(status) != 'REJECTED' THEN timesheet_id END) AS approved_days,\n"
	    		+ "               COUNT(DISTINCT CASE WHEN UPPER(client_approval_status) = 'PENDING' AND UPPER(status) != 'REJECTED' AND NOT EXISTS (SELECT 1 FROM timesheet_document_details WHERE timesheet_id = edsd.timesheet_id AND UPPER(client_approval_status) = 'APPROVED') THEN timesheet_id END) AS pending_days\n"
