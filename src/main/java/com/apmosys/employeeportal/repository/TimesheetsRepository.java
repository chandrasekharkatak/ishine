...
//				+ "        LEFT JOIN Holidays_In_Range h ON adir.dt = h.date_of_holiday\n"
	...
	@Query(value="WITH RECURSIVE\n"
			...
			+ "    COUNT(distinct CASE WHEN ecs.employee_status = 'Approved' THEN ecs.employee_team_map_id  END) AS total_approved_employees,\n"
			+ "    COUNT(distinct CASE WHEN ecs.employee_status = 'Pending' THEN ecs.employee_team_map_id  END) AS total_pending_employees,\n"
			+ "    COUNT(distinct CASE WHEN ecs.employee_status = 'Defaulter' THEN ecs.employee_team_map_id  END) AS total_defaulter_employees,\n"
			+ "    bpe.emp_id, bpe.client_side_id, bpe.start_date, bpe.team_name, bpe.team_id,\n"
			+ "    CASE WHEN bpe.billable_type = 'Shadow' AND s_emp.name IS NOT NULL \n"
			+ "		 THEN CONCAT(bpe.name, ' (Shadow for ', s_emp.name, ')') ELSE bpe.name END AS name,\n"
			+ "    bpe.spoc, bpe.billable_type, bpe.employee_role, bpe.dept_name, bpe.project_id,\n"
			+ "    bpe.project_name, pm.project_manager_name, bpe.po_no, bpe.client_name, bpe.reporting_manager_id,\n"
			+ "    MONTHNAME(dp.from_date) AS month_name,\n"
