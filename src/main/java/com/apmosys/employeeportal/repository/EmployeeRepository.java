	...
	public List<Object[]> getEmployeeViewForClientAttendanceStatus(@Param("status") String status, @Param("month") Integer month, @Param("year") Integer year,@Param("emp_id") Long emp_id,int offset,int pageSize);
	 ...
	 public int updateBillableAndTypeForEmpIds(@Param("billable") String billable,
		...
		public Float getNOOfDays(@Param("empId") Long empId);
		        ...
		        @Query(value ="WITH RECURSIVE Authorized_Employees AS (  \n"
		        		+ "SELECT e.emp_id  \n"
		        		+ "FROM employee e  \n"
		        		+ "WHERE EXISTS (  \n"
		        		+ "	SELECT 1  \n"
		        		...
		        		+ "LEFT JOIN project_overhead_mapping pom ON p.project_id = pom.project_id  \n"
		        		+ "LEFT JOIN employee_team_mapping etm2 ON etm2.team_id = t.team_id  \n"
		        		+ "WHERE :empId IN (pm.project_manager_id, pom.project_overhead_id, t.spoc_id, t.team_lead_id, etm2.emp_id)  \n"
		        		+ ")  \n"
		        		+ "\n"
		        		+ "SELECT DISTINCT  \n"
		        		+ "e.emp_id,  \n"
		        		...
		        		+ "FROM employee e  \n"
		        		+ "JOIN Authorized_Employees ae ON e.emp_id = ae.emp_id  \n"
		        		+ "INNER JOIN employee_team_mapping etm on ae.emp_id = etm.emp_id\n"
		        		+ "INNER JOIN teams t on etm.team_id = t.team_id\n"
		        		+ "INNER JOIN projects p on p.project_id = t.project_id\n"
		        		+ "WHERE e.employmentstatus != 'InActive'  \n"
		        		+ "AND e.emp_id > 6 \n"
		        		+ "and p.has_client_side_id=1 and p.active = 'true' and etm.active = 1 and t.is_active = 'Y'",nativeQuery = true)
