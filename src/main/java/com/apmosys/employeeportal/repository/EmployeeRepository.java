...
//    public void updateLongOverdueNotified1(@Param("empId") Long empId, @Param("status") boolean status);
	...
	@Query(value = " WITH RECURSIVE\n"
			...
			+ "    SELECT et.emp_id, COUNT(DISTINCT et.date) AS filled_working_days\n"
			+ "    FROM employee_timesheets et\n"
			...
			+ "    SELECT\n"
			+ "        tdd.emp_id,\n"
			+ "        COUNT(DISTINCT CASE WHEN UPPER(tdd.client_approval_status) = 'APPROVED' AND tdd.final_flag = 1 THEN et.date END) AS approved_days,\n"
			+ "        COUNT(DISTINCT CASE WHEN UPPER(tdd.client_approval_status) = 'PENDING' AND tdd.final_flag = 1 THEN et.date END) AS pending_days\n"
			...
			+ "    LEFT JOIN Apmosys_Timesheet_Summary ats ON e.emp_id = ats.emp_id\n"
			+ ")\n"
			+ "\n"
			+ "SELECT DISTINCT\n"
			...
			+ "WHERE (:status = 'All' OR employee_status = 'All')\n"
