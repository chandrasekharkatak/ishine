...
//    public void updateLongOverdueNotified1(@Param("empId") Long empId, @Param("status") boolean status);
	...
	@Query(value = " WITH RECURSIVE\n"
			...
			+ "                    COALESCE(ecsd.expected_dsr_days, 0) AS expected_client_side_dsr,\n"
	...
	@Query(value = "WITH RECURSIVE\n"
			...
			+ "    CROSS JOIN All_Dates_In_Range adir\n"
			+ "    LEFT JOIN holiday h ON h.date_of_holiday = adir.dt\n"
			+ "    LEFT JOIN employee_timesheets et_leave ON et_leave.emp_id = brd.emp_id\n"
			+ "                                           AND et_leave.date = adir.dt\n"
			+ "                                           AND upper(et_leave.day_type) LIKE '%LEAVE%'\n"
			+ "    WHERE adir.dt < CURDATE()\n"
			...
			+ "        et.emp_id,\n"
			+ "        et.project_id,\n"
			+ "        COUNT(DISTINCT et.date) AS filled_ishine_days\n"
			+ "    FROM employee_timesheets et\n"
			+ "    JOIN Date_Parameters dp ON et.date BETWEEN dp.from_date AND dp.to_date\n"
			+ "    WHERE (et.day_type LIKE '%Working%' OR upper(et.day_type) LIKE '%LEAVE%')\n"
			+ "    GROUP BY et.emp_id, et.project_id\n"
			...
			+ "        CASE\n"
			+ "            WHEN GREATEST(0, COALESCE(eiwd.expected_ishine_days, 0) - COALESCE(its.filled_ishine_days, 0)) >= 2 THEN 'Defaulter'\n"
			+ "            WHEN COALESCE(eiwd.expected_ishine_days, 0) > COALESCE(its.filled_ishine_days, 0) THEN 'Pending'\n"
			+ "            ELSE 'Approved'\n"
			+ "        END AS employee_status\n"
			+ "    FROM Base_Report_Details brd\n"
			...
			+ "    LEFT JOIN Ishine_Timesheet_Summary its ON brd.emp_id = its.emp_id AND brd.project_id = its.project_id\n"
			+ ")\n"
			+ "SELECT\n"
			+ "    name, emp_id, project_name, po_no, client_name, team_name, project_type,\n"
			+ "    team_lead_name, billable, billable_type, mobile_no, email,\n"
			+ "    expected_ishine_timesheet_days,\n"
			+ "    filled_ishine_timesheet_days,\n"
			...
			+ "ORDER BY name \n"
			+ "LIMIT :offset, :pageSize",nativeQuery = true)
		public List<Object[]> getEmployeeViewForAllEmpAttendanceStatus(@Param("status") String status, @Param("month") Integer month, @Param("year") Integer year,@Param("emp_id") Long emp_id, int offset, int pageSize);
